...
"""  This script is to simulate Anzurez observer with the model in 
...
#Fuction tha contains the script original model
def obs_model(t,x):
    ...
    phi = k*np.sign(np.matrix(x[4:6])*P).T
    ...
    xdot[2:4] = (A*np.matrix(x[2:4]).T + B*u + L.T*(y_e[0,0]) + phi ).T.tolist()[0] #+phi
    xdot[4:6] = ((A- L.T*C)*np.matrix(x[4:6]).T - phi ).T.tolist()[0] #-phi
...
def obs_model_uncer(t,x):
    ...
    #Discontinouos vector
    phi = k*np.sign((np.matrix(x[0:2])-np.matrix(x[2:4]))*P).T
    ...
    #Calculus of the dynamics of the system
    xdot[0:2] = (A*np.matrix(x[0:2]).T + B*u).T.tolist()[0]
    xdot[2:4] = (A_uncer*np.matrix(x[2:4]).T + B*u + L.T*(y_e[0,0]) + phi ).T.tolist()[0] #+phi
...
k = 2
u = 0
phi1 = [(k*np.sign(np.matrix([[x5[0],x6[0]]])*P).T).tolist()[0]]
phi2 = [(k*np.sign(np.matrix([[x5[0],x6[0]]])*P).T).tolist()[1]]
...
while r.successful() and r.t < tf:
    ...
    x6.append(r.y[5])
    t.append(r.t)
    phi1.append((k*np.sign(np.matrix([[r.y[4],r.y[5]]])*P).T).tolist()[0])
    phi2.append((k*np.sign(np.matrix([[r.y[4],r.y[5]]])*P).T).tolist()[1])
       
#plot results

#Figure 1 plot the system vs observer
labels = ['$x_1$','$x_2$',r'$\xi_1$',r'$\xi_2$']
plt.figure()
...
plt.subplot(211)
phi_1 = plt.plot(t,phi1)
plt.xlabel('time')
plt.ylabel('magnitude')
plt.grid(True)
plt.legend(phi_1 ,[labels[0]])
plt.subplot(212)
phi_2 = plt.plot(t,phi2)
plt.xlabel('time')
plt.ylabel('magnitude')
plt.grid(True)
plt.legend(phi_2 ,[labels[1]])
...
while r.successful() and r.t < tf:
    ...
    t.append(r.t)
    phi1.append((k*np.sign(np.matrix([[r.y[0]-r.y[2],r.y[1]-r.y[3]]])*P).T).tolist()[0])
    phi2.append((k*np.sign(np.matrix([[r.y[0]-r.y[2],r.y[1]-r.y[3]]])*P).T).tolist()[1])
